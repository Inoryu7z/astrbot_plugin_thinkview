...
### 🐛 Bug 修复
- 🔧 修复 `full_agent_loop` 模式下工具调用参数、结果、回复摘要和总输出仍被截断的问题（既然用户选择了完整记录，就不应截断）

## 1.3.0 — 2026-04-25

### ✨ 新功能
- 🆕 新增 `/think_here`（别名 `/思考这里`）命令，强制将思考记录输出到当前对话，无论是否配置中转群
- 🆕 新增 `force_local_output` 配置项，开启后 `/think` 和 `/think_search` 指令获取的记录只发送到当前对话，不转发到中转群；自动转发模式不受影响

## 1.2.1 — 2026-04-25

### 🐛 Bug 修复
- 🔧 修复 `_conf_schema.json` 中 `"type": "integer"` 导致插件加载失败的问题（AstrBot 配置解析器仅支持 `"int"`，回退为原始值）
- 🔧 新增中转群 session 格式验证，拼写错误（如 `dafault`）会提前警告而非静默失败
